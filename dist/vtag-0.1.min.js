...
k),Math.round(255*g),Math.round(255*h)]};p.vtag.hsv2hex=function(a,g,h){a=p.vtag.hsv2rgb(a,g,h);for(var k in a)g=a[k].toString(16),a[k]=1==g.length?"0"+g:g;return"#"+a.join("")};p.vtag.renderers.retro=function(a){function g(a,b){h.fillStyle=s;h.beginPath();h.rect(a*m,b*m,m,m);h.fill()}var h=this.getContext("2d"),k=a.width,l=a.height,m=(k+l)/16,n,f,r;n=360*a.bytes[0]/255;f=40*a.bytes[1]/255+60;r=30*a.bytes[2]/255+60;var q=p.vtag.hsv2hex(n,f,r);n+=30*(a.bytes[4]%12);f=50*a.bytes[5]/255;r=20*a.bytes[6]/
255+80;var s=p.vtag.hsv2hex(n%360,f,r);a.inverse&&(n=q,q=s,s=n);a.transparent||(h.beginPath(),h.rect(0,0,k,l),h.fillStyle=q,h.fill());for(k=0;8>k;k++)l=a.bytes[k]%3,q=a.bytes[k+1]%6,g(l+1,q+1),g(6-l,q+1)}})(jQuery);
